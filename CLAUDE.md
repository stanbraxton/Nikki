...
- `app/config.py`: `NIKKI_MODEL` defaults to `anthropic:claude-sonnet-5` (primary). `NIKKI_ENGINEER_MODEL` defaults to `anthropic:claude-sonnet-5` and routes engineering turns via `route_model()`; empty disables routing. It is deliberately the *same* model as `NIKKI_MODEL` for now, so routing stays exercised at no extra cost — extended thinking and the sonnet-5 bump ship as one measurable change, and raising this to an Opus-class model is a separate, separately-measured decision. `NIKKI_FALLBACK_MODEL` defaults to `openai:gpt-4.1` — it must stay on a *different* provider than `NIKKI_MODEL`, or it fails on the same exhausted account.
- **Reasoning upgrade (2026-09-25):** `NIKKI_ENGINEER_MODEL` now defaults to `anthropic:claude-opus-5-5` with `NIKKI_ENGINEER_THINKING_EFFORT=high`; `route_model()` also routes data-analysis/backtest wording there. Chat stays on `NIKKI_MODEL`. Every `repo_commit_push` diff is reviewed first by `NIKKI_REVIEW_MODEL` (default Opus 5.5); blocking findings refuse the push unless `override_review=True` (must be justified). The review fails open on outage. Working-method checklists: top of `knowledge/nikki-system.md` and `knowledge/research-method.md`. Covered by `tests/test_reasoning_upgrade.py`.
- **Adaptive routing (2026-09-25, `tests/test_adaptive_routing.py`):**
  - *Hints are whole words* (`_ENGINEER_HINT_RE`). Substring matching sent "report", "committee" and "pushback" to Opus.
  - *Stickiness comes from the thread, not the browser session.* `recent_thread_tools(await thread_messages(cp, config))` reads the tools called in the thread's last `ROUTE_STICKY_TURNS` (3) user turns straight from the checkpointer. The old session set never faded and was emptied by a page reload. `recent_user_texts()` makes hint-routed turns sticky too, since analysis work often uses no engineer tool (`db_query`, `jobs`).
  - *Approval resumes are routed.* `_continue_approval` re-routes from the thread before building the graph. Before this, every push and deploy approval dropped the rest of an engineering turn to `NIKKI_MODEL`. Keep the `model == settings.model` guard: an explicit chat-settings pick always wins.
  - *`escalate` tool* (`app/tools/escalation.py`, ungated, **admin-only**, so subscribers can't request Opus-priced turns). The model asks for the engineer model itself. `_drive` and `headless.run_prompt` see the call in the round's pending calls and switch before the rebuild, so the model call that reads the tool result is already on the new model. `_drive` now returns the model it ended on. Only automatic choices are switched (`is_auto_model`), never an explicit pick or the billing fallback, and not when the batch holding `escalate` was rejected.
  - *`BUILD_PLAN.md` rule.* It is one sentence in the admin prompt; the detail is item 10 of the `knowledge/nikki-system.md` checklist.
...
- **A billing fallback mid-engineering-turn is its own hazard** — the fallback model will cheerfully keep editing and pushing, and it is not the model the turn was routed to. The fallback was raised from `gpt-4.1-mini` to `gpt-4.1` so a turn that lands there is still usable, but it is still a downgrade from `NIKKI_ENGINEER_MODEL`; consider no fallback on engineering turns so they fail loudly instead.
- **Extended thinking is two different APIs, split by model.** Sonnet 5 / Opus 5 / Opus 4.7+ removed *both* `thinking.type: "enabled"` (with `budget_tokens`) and the sampling parameters — so the original implementation 400'd twice over, and fixing only the thinking dict would have failed again on `temperature`. `make_model()` branches on `uses_adaptive_thinking()`: adaptive models get `reasoning_effort` **alone**, legacy ones (Haiku 4.5, Sonnet 4.5) keep `enabled` + `budget_tokens` + `temperature=1`. An unknown model id defaults to adaptive, so a new ID never inherits the removed API.
  - **Do not "tidy" this by setting `thinking` explicitly on the adaptive path.** Passing `reasoning_effort` alone makes langchain default thinking to `{"type": "adaptive", "display": "summarized"}`. The API default is `"omitted"`, which returns thinking blocks with **empty text** — and `ui.py`'s `stream_segment` renders a block only when `block["thinking"]` is truthy. Setting it by hand buys a request that succeeds and renders nothing, which looks exactly like thinking being off.
  - `NIKKI_THINKING_BUDGET_TOKENS` is the on/off switch for both kinds; it is only a real budget on legacy models. Depth on adaptive models comes from `NIKKI_THINKING_EFFORT` (`low`/`medium`/`high`/`xhigh`/`max`; `high` is the API default, so `medium` is what actually reduces spend). Covered by `tests/test_thinking_config.py`, whose negative cases are the load-bearing ones.
- **`make_model()` logs the resolved model spec on every construction**, and `route_model()` logs why it upgraded a turn. Routing can silently change which model a turn runs on; without those lines an unexplained bill has nothing to attribute it to. Keep them.
...
- **Prompt cache — keep every request a byte-identical extension of the last** *(2026-09-23)*. Request order is tools → system (stable, cached) → system (volatile) → messages. Anything that changes early invalidates everything after it. Three past breakers, all fixed: the volatile block carried HH:MM (now date only; the clock rides on user messages via `stamp_latest_user_message`, memoized per message id); `trim_history` moved its cut every turn once over budget (now cuts only at fixed checkpoints every `budget//2` tokens from the thread start); skill tools were appended in dict order (now sorted). Don't put anything time-varying in the system prompt. `token_usage` now records `cache_read_tokens` / `cache_creation_tokens` — use them to confirm hits. Covered by `tests/test_prompt_cache.py`.
- **Daily cap:** `NIKKI_DAILY_TOKEN_CAP` (20M per tenant since 00:00 UTC, 0 = off) is checked at the start of every chat and headless turn (`guards.daily_cap_message`). It exists because of the 9/19 runaway (~39M tokens in a day). **Light model:** `NIKKI_LIGHT_MODEL` (off by default) routes short non-engineering turns to a cheaper model via `route_light_model`.
