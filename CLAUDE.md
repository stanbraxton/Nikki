...
- `app/config.py`: `NIKKI_MODEL` defaults to `anthropic:claude-sonnet-5` (primary). `NIKKI_ENGINEER_MODEL` defaults to `anthropic:claude-opus-5` and routes engineering turns via `route_model()`; empty disables routing. `NIKKI_FALLBACK_MODEL` defaults to `openai:gpt-4.1` — it must stay on a *different* provider than `NIKKI_MODEL`, or it fails on the same exhausted account.
...
- **A billing fallback mid-engineering-turn is its own hazard** — the fallback model will cheerfully keep editing and pushing, and it is not the model the turn was routed to. The fallback was raised from `gpt-4.1-mini` to `gpt-4.1` so a turn that lands there is still usable, but it is still a downgrade from `claude-opus-5`; consider no fallback on engineering turns so they fail loudly instead.
