...
gcloud run deploy "$SERVICE" --image "$IMAGE" --region "$REGION" --platform managed \
  ...
  --set-env-vars "NIKKI_ENV=prod,ADMIN_USERNAME=stan,NIKKI_WORKSPACE_DIR=/mnt/data/workspace,NIKKI_SKILLS_DIR=/mnt/data/skills,NIKKI_MODEL=${NIKKI_MODEL:-anthropic:claude-sonnet-4-5},SPACES_PROJECT=${PROJECT},SPACES_REGION=${REGION},SPACES_SERVICE_ACCOUNT=nikki-spaces@${PROJECT}.iam.gserviceaccount.com,SCHEDULER_SERVICE_ACCOUNT=nikki-scheduler@${PROJECT}.iam.gserviceaccount.com,PUBLIC_URL=https://${DOMAIN},KNOWLEDGE_DIR=/mnt/data/knowledge,ENGINEER_REPOS_DIR=/tmp/repos,PROJECT_NUMBER=${PROJECT_NUMBER},SIGNUPS_ENABLED=${SIGNUPS_ENABLED:-true},SIGNUP_CODE=${SIGNUP_CODE:-}" \
  --set-secrets "ANTHROPIC_API_KEY=ANTHROPIC_API_KEY:latest,OPENAI_API_KEY=OPENAI_API_KEY:latest,ADMIN_PASSWORD_HASH=ADMIN_PASSWORD_HASH:latest,CHAINLIT_AUTH_SECRET=CHAINLIT_AUTH_SECRET:latest,ADMIN_API_TOKEN=ADMIN_API_TOKEN:latest,DATABASE_URL=DATABASE_URL:latest,GOOGLE_OAUTH_CLIENT_ID=GOOGLE_OAUTH_CLIENT_ID:latest,GOOGLE_OAUTH_CLIENT_SECRET=GOOGLE_OAUTH_CLIENT_SECRET:latest,TAVILY_API_KEY=TAVILY_API_KEY:latest,GITHUB_TOKEN=GITHUB_TOKEN:latest,RESEND_API_KEY=RESEND_API_KEY:latest" \
  -q
