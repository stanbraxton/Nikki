...
gcloud run deploy "$SERVICE" --image "$IMAGE" --region "$REGION" --platform managed \
  ...
  --set-env-vars "NIKKI_ENV=prod,ADMIN_USERNAME=stan,NIKKI_WORKSPACE_DIR=/mnt/data/workspace,NIKKI_SKILLS_DIR=/mnt/data/skills,NIKKI_MODEL=${NIKKI_MODEL:-anthropic:claude-sonnet-4-5},SPACES_PROJECT=${PROJECT},SPACES_REGION=${REGION},SPACES_SERVICE_ACCOUNT=nikki-spaces@${PROJECT}.iam.gserviceaccount.com,SCHEDULER_SERVICE_ACCOUNT=nikki-scheduler@${PROJECT}.iam.gserviceaccount.com,PUBLIC_URL=https://${DOMAIN}" \
