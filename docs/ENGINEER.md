...
| `deploy_self` | approval | Fallback only — pushes to `main` auto-deploy via trigger `nikki-main-deploy`. Cloud Build: docker build → Artifact Registry → new revision of the `nikki` Cloud Run service (repo `cloudbuild.yaml`) |
