...
- **Fresh chat = recover the session first.** Prep conversations live in Postgres (Chainlit): `SELECT id,name,"createdAt" FROM threads ORDER BY "createdAt" DESC LIMIT 10;` then `SELECT type,output FROM steps WHERE "threadId"='<id>' AND type IN ('user_message','assistant_message') ORDER BY "createdAt";` via `db_query`. Harvest from that transcript, not from a summary file.
