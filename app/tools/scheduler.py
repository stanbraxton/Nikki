...
def schedule_task(name: str, cron: str, prompt: str, timezone: str = "America/New_York", auto_approve: bool = False) -> str:
    ...
    if not NAME_RE.match(name):
        return "error: name must be 2-41 chars, lowercase letters/digits/hyphens, starting with a letter"
    ...
    with _e().connect() as c:
        if c.execute(select(s.c.name).where(s.c.name == name)).first():
    ...
    common = [
        ...
        "--attempt-deadline", "180s", "--description", f"Nikki schedule {name}",
    ]
    res = _gcloud("scheduler", "jobs", "create", "http", _job(name), *common)
    if res.returncode != 0 and "already exists" in res.stderr:
        res = _gcloud("scheduler", "jobs", "update", "http", _job(name), *common)
    if res.returncode != 0:
        return f"error: Cloud Scheduler: {res.stderr.strip()[-600:]}"
    with _e().begin() as c:
        c.execute(insert(s).values(name=name, cron=cron.strip(), timezone=timezone, prompt=prompt.strip(),
                                   job_name=f"projects/{project()}/locations/{region()}/jobs/{_job(name)}",
...
def list_schedules() -> str:
    ...
    with _e().connect() as c:
        rows = c.execute(select(s).order_by(s.c.created_at)).fetchall()
        out = []
        for row in rows:
            d = dict(row._mapping)
            last = c.execute(select(r.c.status, r.c.started_at).where(r.c.schedule == d["name"])
...
def schedule_runs(name: str, limit: int = 5) -> str:
    ...
    with _e().connect() as c:
        rows = c.execute(select(r).where(r.c.schedule == name).order_by(r.c.started_at.desc())
...
def run_schedule_now(name: str) -> str:
    ...
    res = _gcloud("scheduler", "jobs", "run", _job(name), "--location", region())
    if res.returncode != 0:
...
def delete_schedule(name: str) -> str:
    ...
    res = _gcloud("scheduler", "jobs", "delete", _job(name), "--location", region())
    if res.returncode != 0 and "NOT_FOUND" not in res.stderr and "not found" not in res.stderr.lower():
        return f"error: {res.stderr.strip()[-400:]}"
    with _e().begin() as c:
        c.execute(delete(persistence.schedules).where(persistence.schedules.c.name == name))
